...
const sendOTP = async function (req, res,next)  {
    ...
    await springedge.messages.send(req.session.params, 5000, function (err, response) {
        if (err) {
...
const registerUser = async function (req,res,next){
   ...
   let checkuser = await user.findOne({$or:[{email:req.body.email},{phone:req.body.mobile}]})
...
const  otpVerify = async function(req,res) {
    ...
    let userOtpDetails= await userOtp.findOne({user_id:req.session.currentUserId})
     ...
     {
        res.render('user/otp',{mobileNumber:req.session.mobileNumber, alert:"OTP Wrong"})
        console.log("wrong");
...
const verifyUser = async function (req, res) {
    ...
    try {
        ...
        {
            let cryptResult = await bcrypt.compare(password, userData.password_encrypted)
